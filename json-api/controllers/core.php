  ...
  public function get_category_index() {
    ...
    $categories = get_categories($args);
       ...
       foreach( $categories as $single_cat ) { 
            if($single_cat->parent < 1) {
              array_push($categories_array, array('image'=>z_taxonomy_image_url($single_cat->term_id), 'name'=>$single_cat->name , 'term_id'=> $single_cat->term_id));
            }
       }

    return array(
      'Parent Category' => $categories_array
    );
  }
    ...
    $categories_array = array();
     ...
     array_push($categories_array, array('image'=>z_taxonomy_image_url($category->term_id), 'name'=>$category->name , 'term_id'=> $category->term_id));  
    }

    return array(
      'Child Category' => $categories_array
    );
  }
